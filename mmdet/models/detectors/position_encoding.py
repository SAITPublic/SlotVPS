...
# import mmdet
from mmdet.core.utils.misc import NestedTensor, trunc_normal_
...
class RelPos(nn.Module):
    ...
    def forward(self, q, attn):
        ...
        onehot = self.map.to(device=q.device, dtype=q.dtype)
        rel_emb_h = self.rel_emb_h.to(device=q.device, dtype=q.dtype)
        rel_emb_w = self.rel_emb_w.to(device=q.device, dtype=q.dtype)

        abs_pos_h = torch.einsum("q k r, r c -> q k c", onehot, rel_emb_h)
        # abs_pos_h = abs_pos_h[:, None, :, None]
        abs_pos_w = torch.einsum("q k r, r c -> q k c", onehot, rel_emb_w)
        # abs_pos_w = abs_pos_w[None, :, None, :]

        # abs_pos_h = abs_pos_h.repeat(1, self.q_len, 1, self.k_len, 1)
        # abs_pos_w = abs_pos_w.repeat(self.q_len, 1, self.k_len, 1, 1)

        q = rearrange(q, "b (qh qw) p g (n c) -> b qh qw p g n c", qh=self.q_len, n=2)
        logits_h = torch.einsum("b h w p g c, h k c -> b g h w k p", q[..., 0, :], abs_pos_h)
        logits_w = torch.einsum("b h w p g c, w k c -> b g h w k p", q[..., 1, :], abs_pos_w)
        logits_h = rearrange(logits_h, "b g h w k p -> b g (h w) k 1 p")
        logits_w = rearrange(logits_w, "b g h w k p -> b g (h w) 1 k p")

        attn = rearrange(attn, "b g q (kh kw) p -> b g q kh kw p", kh=self.k_len, kw=self.k_len)
        attn += logits_h
        attn += logits_w
        return rearrange(attn, "b g q h w p -> b g q (h w) p")
...
class PositionEmbeddingSinev2(nn.Module):
    ...
    def forward(self, x, H=0, W=0, mask=None):
        B = x.shape[0]
        if self.pos is None:
            mask = torch.zeros((B, H, W), device=x.device, dtype=torch.bool) if mask is None else mask.bool()
